...
|------|--------|----------|
| `valid-resigned-broken-chain-v3.jpg` | Fastly Image Optimizer output (Fastly ← Microsoft Paint, broken signature ← Google AI). Supplied to c2paviewer.com by the reporting engineer at Fastly for regression testing. | file has no errors; chain reports `signingCredential.invalid`, `claimSignature.mismatch`, `ingredient.manifest.mismatch`; two `invalid` nodes. Every failure lives in the **Ingredient V3** `validation_results` wrapper. |
...
respectively and are included for testing only. `valid-resigned-broken-chain-v3.jpg`
